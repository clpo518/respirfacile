...
serve(async (req) => {
  ...
  try {
    const { sessionId, patientId, therapistId, commentPreview }: NotifyCommentRequest = await req.json();

    console.log(`Notifying patient ${patientId} of comment from ${therapistId} on session ${sessionId}`);
    ...
    const emailResult = await sendEmail("therapist_comment", patientAuth.user.email, {
      patientName,
      therapistName,
    ...
    console.log(`Comment notification email sent to ${patientAuth.user.email}`);
    ...
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    console.error("Error in notify-comment:", errorMessage);
    return new Response(
